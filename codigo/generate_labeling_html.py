# codigo/generate_labeling_html.py
import pandas as pd
import json
import os
...
def generate_labeling_html():
    ...
    excel_file = os.path.join(DADOS_DIR, 'Amostra_Rotulagem_ABSA_Balanceada.xlsx')
    if not os.path.exists(excel_file):
        excel_file = os.path.join(DADOS_DIR, 'Amostra_Rotulagem_TCC.xlsx')
        if not os.path.exists(excel_file):
            print(f"Error: No sample file found. Please run generate_sample.py first.")
            return
...
<body>
    ...
    # Write HTML file
    output_file = os.path.join(DADOS_DIR, 'rotulacao_reviews.html')
